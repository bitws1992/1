...
data=analog_daqmx_acceleration_supension_z_right.data;% original signal
time=analog_daqmx_acceleration_supension_z_right.time;% original time
fre_data=round(length(data)/(time(end)-time(1)));
%% 判断采样频率是否为51200
if ~fre_data==51200
    yy=time(1):(1/51200):time(end);
...
for i=1:L
    ...
    xx=time_3(1):(1/51200):time_3(end);
    label=interp1(time_3,label,xx, 'spline');
    if  ~all(label==0)
        index=find(label>0);
